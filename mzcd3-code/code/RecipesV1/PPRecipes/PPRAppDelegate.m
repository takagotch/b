...
@property (nonatomic, strong) PPRDataController *dataController;
...
{
  [self setDataController:[[PPRDataController alloc] initWithCallback:^{
...
{
  if ([[self dataController] managedObjectContext]) {
...
- (void)consumeIncomingFileURL:(NSURL*)url;
{
  ...
  [op setSuccessBlock:^(BOOL success, NSError *error) {
    if (success) {
