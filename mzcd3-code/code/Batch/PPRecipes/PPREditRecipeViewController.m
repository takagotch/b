  ...
  while (index <= 6) {
    ...
    switch (index) {
      ...
      case 6: //ingredients
        ...
        [[cell detailTextLabel] setText:[NSString stringWithFormat:@"%lu", (unsigned long)[temp count]]];
        break;
      default:
        ALog(@"Bad index: %li", (long)index);
    }
    ++index;
  }
}
...
- (void)prepareForSelectIngredientsSegue:(UIStoryboardSegue*)segue sender:(id)sender
{
  ...
  [viewController setUpdateIngredientCountBlock:^(NSInteger ingredientCount) {
    ...
    UITableViewCell *cell = [[self tableView] cellForRowAtIndexPath:path];
    [[cell detailTextLabel] setText:[NSString stringWithFormat:@"%li", (long)ingredientCount]];
  }];
